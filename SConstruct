...
tElf = tEnv.Elf('targets/netx90_com_intram/scatter_load.elf', tSrc + tEnv['PLATFORM_LIBRARY'])
tTxt = tEnv.ObjDump('targets/netx90_com_intram/scatter_load.txt', tElf, OBJDUMP_FLAGS=['--disassemble', '--source', '--all-headers', '--wide'])
tBin = tEnv.ObjCopy('targets/netx90_com_intram/scatter_load.bin', tElf)
...
atSnippet = {
    ...
    'description': 'Copy several chunks and start a firmware.',
    'categories': ['netx90MPW', 'booting', 'SDRAM'],
    'parameter': {
        'LIST': {'help': 'A pointer to the start of the list.'}
    }
}
